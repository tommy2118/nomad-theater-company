...
**Cause:** Tailwind 4 treats `group` as a state marker, not a utility class. You cannot use `@apply group` or `@apply group-hover:*`.

**Fix:** Use standard CSS selectors for group hover effects:

```css
/* Instead of @apply group-hover:scale-105 */
...
7. **Cannot `@apply group`** — In Tailwind 4, `group` is a state marker, not a utility class. Use standard CSS selectors for group hover effects:

```css
/* WRONG - will error in Tailwind 4 */
.card {
